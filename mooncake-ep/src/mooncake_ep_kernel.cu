...
dispatch(void* packed_recv_x, float* packed_recv_x_scales,
    ...
    const size_t hidden_bytes = kHidden * (kUseFP8 ? sizeof(ep_fp8_storage_t) : 2 /*sizeof(nv_bfloat16)*/);
    const size_t hidden_int4 = hidden_bytes / sizeof(int4);

    // Message package: hidden data, FP8 scales, index at source
    // NOTES: currently we have 3 reserved int fields for future use
    using vec_t = typename std::conditional<kUseFP8, int2, int4>::type;
    const size_t num_bytes_per_msg = sizeof(int4) + (kUseFP8 ? (kHidden + num_scales * sizeof(float)) : (kHidden * 2 /*sizeof(nv_bfloat16)*/));
    ...
    if (warp_id < num_warps - 1) {
        constexpr int kNumElemsPerRead = sizeof(int4) / 2 /*sizeof(nv_bfloat16)*/;
...
combine(void* combined_x, int32_t* active_ranks,
    ...
    // Data type staffs
    constexpr int kNumElemsPerInt4 = sizeof(int4) / 2 /*sizeof(nv_bfloat16)*/;
    const size_t hidden_bf16_int4 = kHidden / kNumElemsPerInt4;

    // Message package
    constexpr size_t num_bytes_per_slot = kHidden * 2 /*sizeof(nv_bfloat16)*/;
